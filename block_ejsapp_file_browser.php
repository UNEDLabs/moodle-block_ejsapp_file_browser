...
class block_ejsapp_file_browser extends block_base {
    ...
    /**
    * Defines the javascript code for refreshing the page in which the block is
    * embedded.    .
    *
    * @no params
    * @return string Javascript code for refreshing the page
    */
    function get_javascript_code()
    {
        $javascript_code = <<< End_Of_Javascript
...
End_Of_Javascript;
    ...
    */
    function get_content()
    {
        ...
        // <post-process 'a href' links for xml state files>
        preg_match_all('/<\s*a\s+href\s*=\s*"([^"]*)">/i', $this->content->text, $all_xml_matches);
          ...
          if (preg_match('/\/.+[.]xml/i', $match, $state_file_match)) {
            ...
            $state_file = urldecode($slices[sizeof($slices) - 1]);
              ...
              if ($ejsapp_record) {
                ...
                $this->content->text = preg_replace('/' . $reg . '/', $new_code, $this->content->text);
                // replace icon
                preg_match("/(<img.+$state_file.+src=[\"|']).+([\"|'].+>)/Ui", $this->content->text, $image_match);
